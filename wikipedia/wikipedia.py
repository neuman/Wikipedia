...
class WikipediaPage(object):
  ...
  def sections_tree(self, add_node_callback):
    '''
    List of sections titls from the table of contents on the page,
    along with their level information.
    '''

    if not getattr(self, '_sections_hierarchy', False):
      query_params = {
        'action': 'parse',
        'prop': 'sections',
        'page': self.title
      ...
      for section in request['parse']['sections']:
        add_node_callback(section['line'], section['toclevel'])
